...
func ProvideDBConn(config *configs.DatabaseConfig) (*gorm.DB, error) {
	...
	args := fmt.Sprintf(
		"host=%s port=%d user=%s dbname=%s password=%s sslmode=%s connect_timeout=%d",
		host, port, username, dbName, password, sslMode, connectionTimeout,
	)
	...
	if err != nil {
		return nil, errors.WithMessagef(err, "Setup.db.DB")
	}
